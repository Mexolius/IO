...
export class AddThresholds extends Component<IProps, IState> {
  ...
  render() {
    switch (this.state.status) {
      case 200:
      return (<h2>You changed grade's thresholds!</h2>)
                    ...
                    <div className="w3-large"> 
                      {
                        this.state.thresHolds.map((item, index) =>{
                          return <input className="w3-larg w3-margin" onChange={(event) => this.modifyNodeThresholds(event, rowInfo, index)} type="number" step="0.1" min="0" max="1" placeholder={item} />
                        })
                      }
                    </div>
                  ) 
