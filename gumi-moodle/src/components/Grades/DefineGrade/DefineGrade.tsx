...
export class DefineGrade extends Component<IProps, IState> {
  ...
  getFlatData() {
    ...
    var flatTree = treeData.map(
      (e, key) => {
        return {
          _id: 'grade_' + key, name: e.node.title, level: e.path.length - 1, maxPoints: e.node.points, isLeaf: e.node.isLeaf, studentPoints: {}, thresholds: [], parentID: this.getParentKey(e.path)
        }
      }
    )
